...
Med en knækfrekvens på \SI{159}{Hz} og en hældning på \SI{-40}{dB/dekade} er dæmpningen ved \SI{10}{kHz} på størrelsesordenen $10^{-4}$, hvilket reducerer risikoen for aliasering af højfrekvent støj.
...
Filteret er et andenordens Sallen-Key lavpasfilter i unity-gain-konfiguration ($K=1$) med $R_1=R_2=R$ og $C_1=C_2=C$. Knækfrekvensen er givet ved $f_c = 1/(2\pi R C) \approx \SI{159}{Hz}$, og kvalitetsfaktoren bliver $Q = 1/(3-K) = 0.5$ \parencite{Karki2002}. Filteret er dermed overdæmpet i forhold til en Butterworth-respons ($Q = 1/\sqrt{2} \approx 0.707$), hvilket giver et blødere knæk og en \SI{-3}{dB}-frekvens lavere end $f_c$.

Magnituden ved en given frekvens kan beregnes som:
\begin{equation}
    |H(f)| = \frac{K}{\sqrt{\left[1 - (f/f_c)^2\right]^2 + \left[(3-K)(f/f_c)\right]^2}}
\end{equation}
