...
Ved \SI{50}{Hz} fås $|H(\SI{50}{Hz})| \approx 0.91$, dvs.\ en dæmpning på ca.\ \SI{0.8}{dB}. Lavpasfilteret undertrykker derfor kun \SI{50}{Hz}-støj marginalt. Den dominerende attenuering kommer fra software-midlingen over heltallige perioder af nettet, jf.\ ligning \ref{eq:midling_50Hz_integral} til \ref{eq:midling_50Hz_cos}.

En lavere analog knækfrekvens ville desuden være upraktisk, da den ville kræve aflodning af de eksisterende \SI{1}{nF}-kondensatorer med tilhørende risiko for termisk skade på PCB'en. Resterende DC og langsom drift fra ambient-lys reduceres efterfølgende via mørkesubtraktion i software, som beskrevet i afsnit \ref{sec:signalbehandling}.
...
med et nulfase Butterworth-båndpas af anden orden i hjertebåndet \SIrange{0.5}{2}{Hz} (\SIrange{30}{120}{BPM}). AC-amplituden estimeres robust som forskellen mellem 95.- og 5.-percentilen, hvilket dæmper enkeltstående bevægelsesartefakter. Forholdet
\begin{equation}
...
De analyserede billeder blev behandlet som 8-bit intensitetsbilleder, hvor hver pixelværdi ligger i intervallet 0--255. Intensitetsværdierne skal derfor forstås som kvantiserede kamera-counts og ikke som kontinuerte radiometriske målinger. Dette er især relevant ved lave signalniveauer, hvor et enkelt count kan udgøre en ikke-negligerbar relativ ændring.
...
For hver bølgelængde blev der beregnet både median og gennemsnit af den mørkekorrigerede intensitet i ROI'et. Medianen blev anvendt som primær statistik, mens gennemsnittet blev anvendt som kontrol.
...
\end{equation}
Da den absolutte koblingseffektivitet og geometri ikke var kendt, blev signalet normaliseret til \SI{940}{nm}:
\begin{equation}
...
De spektrale korrektionsfaktorer for Glo-bar, linser, LVF og kameraets kvanteeffektivitet er samlet i tabel \ref{tab:hyperspectral_correction} i appendiks. Disse værdier blev anvendt til beregning af den relative, spektralt korrigerede fingertransmission.
