...
\end{equation}
hvor: \\
$\varepsilon(\lambda)$ er den molare absorptionskoefficient, også kaldet ekstinktionskoefficient og er en materialegenskab der beskriver hvor stærkt et mol af et stof absorberer lys ved en given bølgelængde, angives i enheden $\si{cm^{-1}.M^{-1}}$ \\
...
bølgelængder bliver:
\begin{equation}
    \frac{I_{\text{Rayleigh}}(660\,\mathrm{nm})}{I_{\text{Rayleigh}}(940\,\mathrm{nm})} = 
...
$G$ er et dimensionsløst, ikke-absorberende tabsled, der samler bidrag fra spredning, optisk kobling og målegeometri\\
$\varepsilon C l$ er bidraget fra absorption.

Ved transmissionsmåling kan $G$ fortolkes som den del af attenuationen, der skyldes fotoner, som ikke registreres af detektoren selvom de ikke nødvendigvis er absorberet af hæmoglobin. Det kan eksempelvis skyldes at fotoner spredes ud af detektorens optiske accept, tab i den mekaniske geometri eller ændret kobling mellem LED, finger og fotodiode. $G$ afhænger derfor af hele måleopsætningen og er ikke en simpel funktion af anisotropifaktoren $g$ alene.
...
Ved at betragte forholdet mellem intensiteterne ved systole og diastole kan afhængigheden af flere ukendte parametre reduceres. Intensiteterne udtrykkes via ligning \ref{eq:mbbl} \cite[s.126--129]{Webster1997}. Her anvendes $\varepsilon = \varepsilon(\lambda, S)$ som defineret i ligning \ref{eq:epsilon_mix}. Over en enkelt pulsperiode antages $S$ konstant mellem systole og diastole, da det er blodvolumen og ikke iltmætningen, der giver den pulsatile variation i PPG-signalet. Fysiologiske ændringer i iltmætning kan naturligvis forekomme over længere tid, men de optræder ikke som en instantan funktion af selve pulsslaget og kan desuden observeres forsinket i perifere målinger \cite[s.~7]{YossefHay2018}:
\begin{equation}
...
\begin{tabular}{lccc}
\toprule
Måling & 940 nm LED & 660 nm LED & Mørkemåling\\
\midrule
Måling 1 & Tændt & Slukket & Nej \\
Måling 2 & Slukket & Tændt & Nej \\
Måling 3 & Slukket & Slukket & Ja \\
\bottomrule
\end{tabular}
\end{table}
...
\end{equation}
svarende cirka til den røde LED anvendt i dette projekt. For MQW-baserede LED'er er den relevante overgangsenergi bestemt af både materialesammensætning og kvantebrøndstruktur, og kan derfor ikke altid angives som en simpel bulk-materialekonstant. Den anvendte \SI{940}{nm} GaAlAs MQW-LED svarer dog til en effektiv fotonenergi på
\begin{equation}
    E_{940} \approx \frac{1240}{940} \approx \SI{1.32}{eV}.
\end{equation}
Tabel \ref{tab:faststoffer} viser båndgab og typiske anvendelser for relevante materialer.

\begin{table}[h]
\centering
\caption{Båndgabsenergier for udvalgte halvledermaterialer ved 
\SI{300}{K}, samt typiske anvendelser. Materialeværdierne er fra 
\cite[Tabel~16.3-1]{Saleh2007}. For \SI{940}{nm} GaAlAs MQW-LED'en angives den effektive fotonenergi beregnet fra emissionsbølgelængden.}
\label{tab:faststoffer}
\begin{tabular}{@{}lSll@{}}
\toprule
Materiale & {$E$ [eV]} & Type & Typisk anvendelse \\
\midrule
...
GaAlAs MQW      & 1.32 & Direkte   & IR LED (\SI{940}{nm}) \\
...
\begin{figure}[htbp]
     ...
     direkte (a) og indirekte (b) båndgab. Figuren er justeret efter undervisningsmateriale \cite[slide~28]{Jakobsen2025LightSources}.}
...
Et LED-datablad angiver typisk peak-bølgelængde, spektral bredde, fremadspænding, optisk udstråling, stige- og faldtid samt flere elektriske og optiske grænseværdier. De radiometriske størrelser behandles samlet i næste afsnit, mens de vigtigste elektriske databladspunkter for dette projekt opsummeres her:


\begin{itemize}
    \item Fremadspænding \\ \\
    Fremadspændingen $V_f$ er den spænding, der typisk falder over LED'en ved en given fremadstrøm. LED'en drives normalt med en ønsket strøm, mens den resulterende spænding over junctionen er $V_f$.

    Fremadspændingen $V_f$ varierer mellem bølgelængder. En IR-LED kan eksempelvis have en fremadspænding omkring \SI{1.3}{V}, mens en blå LED kan ligge omkring \SI{3.3}{V}. Forskellen skyldes primært halvledermaterialets båndgab. Materiale og dotering bestemmer samtidig LED'ens peak-bølgelængde og spektrale bredde \cite[slide~28]{Jakobsen2025LightSources}.
    ...
    \item Rise- og falltime \\

    En LED er opbygget af to modsat dopede halvledere, n-type og p-type, hvor der omkring pn-overgangen dannes et depletionsområde uden frie ladningsbærere. Området adskiller ladninger på hver side af overgangen, og LED'en kan derfor i praksis have en junction-kapacitans. Denne kapacitans angives ofte i databladet og ligger typisk i størrelsesordenen \SIrange{10}{100}{pF}.
    
    Kapacitansen betyder, at LED'en ikke kan tænde og slukke øjeblikkeligt, da ladning skal flyttes ind og ud af junctionen. Dette angives typisk i databladet som rise- og falltime. De to tider er ofte af samme størrelsesorden:
    $$
    t_{\mathrm{rise}} \approx t_{\mathrm{fall}}
    $$
...
Mere avancerede LED'er kan desuden have et eller flere kvantebrøndslag mellem n-type og p-type, kaldet quantum wells eller multiple quantum wells (MQW). Disse strukturer kan forbedre den interne effektivitet og gøre det muligt at opnå højere optisk output ved en given strøm.
...
Når lys rammer i eller nær depletion region i fotodioden, dannes elektron-hul par. Elektronerne drives mod n-siden, katoden, mens hullerne drives mod p-siden, anoden. Det svarer til en konventionel fotostrøm internt i fotodioden fra katode mod anode. I den anvendte TIA-konfiguration er fotodiodens anode forbundet til jord, mens katoden er forbundet til operationsforstærkerens inverterende indgang. Den lysgenererede strøm trækker derfor konventionel strøm fra summeknuden ved den inverterende indgang, gennem fotodioden og videre til GND.
...
Til TIA-ligningen defineres $I_{\mathrm{PD}}$ som den algebraiske fotodiodestrøm med positiv reference ind i summeknuden ved op-ampens inverterende indgang. I den viste fotodiodeorientering er den fysiske lysgenererede strømretning modsat denne reference, da strømmen løber fra summeknuden gennem fotodioden til GND. Ved belysning bliver $I_{\mathrm{PD}}$ derfor negativ. TIA'ens standardrelation er:
\begin{equation}
    V_{out}=-I_{\mathrm{PD}}R_f
\end{equation}
For den viste orientering giver den negative $I_{\mathrm{PD}}$ derfor en positiv udgangsspænding ved stigende indfaldende lysintensitet. Pilene i figur \ref{fig:PD_ingen_bias_TIA} viser den fysiske strømvej fra op-ampens output gennem feedbackmodstanden, videre gennem fotodioden og til GND.
...
\begin{equation}
    E_e \left[\frac{W}{\text{m}^2}\right]= \frac{\Phi_{\mathrm{opt}}}{A} 
\end{equation}
...
Hyperspektral billeddannelse registrerer en intensitet som funktion af to rumlige koordinater og bølgelængde, dvs. en datakube $I(x,y,\lambda)$ \cite{Tuchin2015}.
    ...
    I dette projekt blev hyperspektral information opnået ved at måle et spektralt bånd ad gangen. En bredbåndet Glo-bar belyste fingeren, hvorefter det transmitterede lys blev ført gennem et lineært variabelt filter. Ved at flytte filteret med en lineær stage blev forskellige smalle spektrale bånd udvalgt og registreret med kameraet.
...
Det pulsatile PPG-signal følger hjertets frekvens, typisk \SIrange{0.5}{2}{Hz} svarende til 30 til 120 slag per minut \cite{Allen2007,Ruberry2025}. I dette system skal der skelnes mellem ADC'ens rå samplingfrekvens og den effektive målefrekvens efter tidsdemultipleksering. AD3'en sampler den analoge spænding kontinuerligt med \SI{20}{kHz}, hvilket giver mange rå samples inden for hvert \SI{40}{ms}-målevindue. Disse samples midles derefter til en enkelt værdi for den aktuelle fase.

Efter demultipleksering fås en separat tidsserie for hver måletype, hvor samme måling kun optræder en gang per komplet målecyklus. Da en cyklus består af tre faser og varer \SI{135}{ms}, bliver den effektive samplingfrekvens for hver demultiplekset tidsserie
\begin{equation}
    f_{\mathrm{serie}} = \frac{1}{\SI{135}{ms}} \approx \SI{7.41}{Hz}.
\end{equation}
Det er denne målefrekvens, ikke $\SI{20}{kHz}/3$, der er relevant for Nyquist-vurderingen af det rekonstruerede PPG-signal. Nyquist-frekvensen bliver derfor cirka \SI{3.7}{Hz}, hvilket ligger over det forventede pulsinterval i dette projekt.
...
\paragraph{Fotonstøj (shot noise).} Den lysgenererede fotostrøm $I_{\mathrm{PD}}$ består af diskrete ladningsbærere og er derfor forbundet med statistiske fluktuationer. 
Den RMS-støjstrøm er:
\begin{equation}
    i_{\mathrm{shot}} = \sqrt{2 e I_{\mathrm{PD}} B}
    \label{eq:shot_noise}
\end{equation}
...
\begin{equation}
    v_{\mathrm{Johnson}} = \sqrt{4 k_B T R_f B}
...
Temperaturen påvirker både lyskildens og detektorens egenskaber. For LED'er kan emissionsmaksimum forskydes mod længere bølgelængder med stigende junction-temperatur, i størrelsesordenen \SI{0.35}{nm} til \SI{0.6}{nm} per celsius. En større temperaturændring kan derfor ændre de effektive ekstinktionskoefficienter og introducere en systematisk fejl i $R$ \cite[s.~66]{Webster1997}.
