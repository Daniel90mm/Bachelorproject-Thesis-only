...
\begin{description}
    ...
    \end{equation}
    Hvor: \\
    $[\text{Hb}]$ er hæmoglobinkoncentrationen i $\frac{\text{g}}{\text{L}}$ \\
...
Blandt disse parametre betragtes $\text{SaO}_2$ som guldstandarden, idet den bestemmes præcist via blodgasanalyse. Blodgasanalyser kræver imidlertid en arteriel blodprøve med en probe ind i en arterie og er derfor invasive. \\ 
...
I væv er denne antagelse ikke opfyldt, da spredning bidrager væsentligt til attenuationen \cite{Tuchin2015}. Det vil altså sige, at eq. \ref{eq:bbl_fysik} kun gælder når $\mu_a \gg \mu_s$, det vil sige, når man kun har med mere absorberende end spredende medier at gøre. Konsekvenserne af spredning behandles i \cref{subsec:spredning}, og en modificeret formulering af BBL præsenteres i sektion \ref{sec:mbbl}.
\section{Optiske egenskaber af biologisk væv}
\label{sec:optik_vaev}

\subsection{Attenuation}
Attenuation beskriver tabet af lysintensitet gennem et medium. Den kan forklares af to mekanismer: absorption, hvor fotoner destrueres ved energioverdragelse til molekyler, og spredning, hvor fotoner omdirigeres fra deres oprindelige udbredelsesretning. 
...
relevant, da en måling her er uafhængig af iltmætningen og  kan derfor anvendes som referencekanal, fordi signalet her i mindre grad afhænger af iltmætningen, selvom det ikke kan bruges til at skelne mellem $\text{HbO}_2$ og $\text{Hb}$.
...
\paragraph{Rayleigh-spredning} dominerer når partiklerne er væsentligt mindre end bølgelængden. Hæmoglobinmolekylet har en diameter på ca.\ \SI{5}{nm} \cite{Erickson2009}, 100 til 150 gange mindre end de klassisk anvendte bølgelængder (\SI{660}{nm} og \SI{940}{nm}) \cite{Webster1997}, og kan derfor beskrives ved Rayleigh-tilnærmelsen. Spredningens intensitet er proportional med $\lambda^{-4}$ \cite{Pedrotti2017}:
\begin{equation}
...
end NIR-lys ved \SI{940}{nm} fra rene Rayleigh-spredere. Dette forhold er en idealiseret eksempel og beskriver ikke den samlede vævsspredning, som i praksis domineres af større spredere.

\paragraph{Mie-spredning} dominerer når partikelstørrelsen er sammenlignelig med eller større end bølgelængden, dette er relevant for celleorganeller, røde blodlegemer ($\sim$\SI{7}{\micro\meter}) og kollagenfibre \cite{DiezSilva2010} \cite[s.~4-6]{Tuchin2015}. Mie-spredning har en svagere bølgelængdeafhængighed end Rayleigh og er desuden overvejende fremadrettet, i modsætning til den tilnærmelsesvist symmetriske Rayleigh-spredning \cite[s. 13]{Tuchin2015}.

\paragraph{Anisotropi.} Da de dominerende spredere i biologisk væv (celleorganeller, røde blodlegemer og kollagenfibre) alle falder i Mie-regimet, er vævsspredning som helhed ikke isotrop. Anisotropien kvantificeres med faktoren \cite{Jacques2013}:
\begin{equation}
    g = \langle \cos \theta \rangle
    \label{eq:anisotropy}
\end{equation}
hvor $\langle \cdot \rangle$ betegner forventningsværdien over 
alle spredningshændelser, og $\theta$ er spredningsvinklen 
i forhold til indfaldsretningen. Værdien $g = 0$ svarer til isotropisk spredning, $g = 1$ til fuld fremadspredning og $g = -1$ til fuld tilbagespredning. Dette kan visualiseres i figur \ref{fig:anisotropi}:

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Graphs/Anisotropi.png}
    \caption{Illustration af anisotropisk spredning i biologisk væv. Anisotropifaktoren $g = \langle \cos \theta \rangle$ beskriver den gennemsnitlige spredningsretning, hvor $g = 0$ svarer til isotropisk spredning, mens $g \sim 1$ og $g \sim -1$ svarer til henholdsvis fremad- og tilbagespredning. For biologisk væv er $g \approx 0.7$ til $0.95$.}
...
Da $l = \text{DPF} \cdot d$ indgår direkte i absorptionsleddet $\varepsilon C l$ i \cref{eq:mbbl_ln}, skalerer DPF absorptionsleddet $\varepsilon Cl$ proportionalt. En større effektiv vejlængde betyder at fotonerne møder mere hæmoglobin, og dermed vil den geometriske afstand $d$ undervurdere den faktiske optiske vejlængde og dermed ikke beskrive den målte attenuation korrekt. Da spredning er bølgelængdeafhængig, er DPF ikke identisk ved \SI{660}{nm} og \SI{940}{nm}, hvilket har konsekvenser for ratio-of-ratios-modellen, dette behandles i \cref{sec:ratio_of_ratios}.
...
$S$ repræsenterer fraktionen af oxyhæmoglobinfraktionen.
...
\begin{equation}
    ...
    \label{eq:mbbl_so2}
\end{equation}
For at isolere $\text{S}$ fra \cref{eq:mbbl_so2} skal parametrene $G$, $C$ og $l$ kendes. Disse parametre varierer imidlertid betydeligt fra person til person, hvilket gør direkte beregning upraktisk. Pulsoximetri reducerer afhængigheden af ukendte, absolutte baggrundsbidrag ved at udnytte den pulsatile ændring i arterielt blodvolumen.
...
Pulsoximetri kan udføres i to optiske konfigurationer (\cref{fig:målekonfigurationer}). Ved transmissionsmåling placeres lyskilde og detektor på modsatte sider af vævet, typisk fingerspids, øreflip eller hos spædbørn, fod eller tå. Lyset gennemtrænger vævet, og detektoren registrerer den transmitterede intensitet.

Ved refleksionsmåling placeres lyskilde og detektor på samme side, og detektoren registrerer det tilbagespredte lys. Pulsoximeteret placeres normalt på brystet, panden eller håndleddet, hvilket gør konfigurationen mere fleksibel og velegnet til flere  placeringer på kroppen.

\section{Pulsoximetrisk måleprincip}
\label{sec:pulseox_princip}

\subsection{Reduktion af ukendte parametre}
Ved at betragte forholdet mellem intensiteterne ved systole og diastole kan afhængigheden af flere ukendte parametre reduceres. Intensiteterne udtrykkes via \cref{eq:mbbl} \cite[s.126--129]{Webster1997}. Her anvendes $\varepsilon = \varepsilon(\lambda, S)$ som defineret i \cref{eq:epsilon_mix}. $S$ antages konstant mellem systole og diastole, det er blodvolumen, ikke iltmætningen, der varierer med pulsen:
\begin{equation}
...
Målingerne køres i cyklus i rækkefølgen: \SI{940}{nm}, \SI{660}{nm}, mørkemåling. Tabel \ref{tab:maalinger} ovenover viser en komplet målecyklus.
...
Forskellen mellem halvledere, isolatorer og metaller illustreres i \cref{fig:de tre bånd}. Hver klasse repræsenteres med to bånd: et valensbånd og et ledningsbånd. 
...
Forskellen mellem metaller, halvledere og isolatorer ligger i størrelsen af deres båndgab. I metaller er et bånd delvist fyldt eller også overlapper valens- og ledningsbånd, så ladningsbærere kan bevæge sig uden termisk excitation over et båndgab. I isolatorer er båndgabet så stort ($> \SI{4}{eV}$) at termisk excitation ved stuetemperatur ikke er nok for at flytte energiniveauet af elektronerne. Halvledere ligger imellem, med båndgab i intervallet \SIrange{0.5}{3}{eV}, hvilket gør deres konduktivitet kontrollerbar via temperatur, doping og belysning.
...
\label{eq:bølgelængde_energi}
    E = \frac{hc}{\lambda},
\end{equation}
hvor $h = \SI{6.626e-34}{J\cdot s}$ er Plancks konstant og $c$ er lysets hastighed. Med $E$ angivet i eV og $\lambda$ i nm approksimeres udtrykket bekvemt som:
\begin{equation}
...
En LED udsender inkohærent lys, hvilket betyder, at det udsendte elektromagnetiske felt ikke har en veldefineret fase-relation over tid som i en laser. For inkohærent lys er der ingen inbyrdes korrelation mellem fotonerne og derfor er det deres intensitet som skal adderes. For kohærente kilder skal de elektriske felter adderes, hvilket muliggør interferens.

Når n- og p-typen har oprettet ligevægt mellem hinanden, vil et såkaldt forward bias, give et potentiale, der tillader elektronerne i n-type til at rejse til p-typen, imod det eksisterende, modvirkende elektriske felt. Ved forward bias reduceres den indbyggede potentialbarriere over pn-overgangen, så elektroner og huller lettere kan injiceres mod junctionen, dette visualiseres i figur\ref{fig:PN_junction_forward_bias}.
...
\begin{itemize}
    ...
    Fremadspændingen $V_f$ varier mellem bølgelængder, og fremadspændingen er vidt forskellig om man vil drive en IR-LED som har en fremadspænding på $\approx$ \SI{1.3}{V}, mens at en blå LED kan have en fremadspænding på $\approx$ \SI{3.3}{V}. Forskelle i fremadspænding skyldes primæært halvledermaterialets båndgab og dermed det valgte materiale. Hvor forskellige halvledere skal dopes til at få de elektriske egenskaber, som giver den bølgelængde der udsendes.
    ...
    $$
    Så en typisk datasheet værdi for en rise time vil være omkring de 10 til \SI{100}{ns}. Denne værdi bidrager til en øvre grænse for hvor hurtigt man derfor kan multiplekse sine signaler, f.eks. for et pulsoximeter. Men er sandsynligvis ikke den begrænsende faktor.
\end{itemize}
...
\section{Radiometri}
Radiometri beskriver kvantificering af elektromagnetisk stråling i fysiske enheder. I dette projekt er særligt radiant flux, irradians og responsivitet relevante. Radiant flux $\Phi_e$ beskriver den samlede optiske effekt i watt. Irradians $E_e$ beskriver den optiske effekt per areal på en overflade:
\begin{equation}
    E_e=\frac{\Phi_e}{A}.
\end{equation}
...
\begin{figure}[htbp]
    ...
    \caption{Fotodiode koblet i fotovoltaisk tilstand til en transimpedansforstærker. Den viste orientering giver positiv udgangsspænding for stigende indfaldende lysintensitet.}
...
Det effektive fotosensitive areal, er det totale brugbare areal som en fotodiode kan opfange fotoner på.  Den relevante optiske parameter for det indkommende lys er irradians:
\begin{equation}
...
Responsiviteten $R_{\lambda}$ angiver forholdet mellem fotostrøm og indfaldende optisk effekt ved en given bølgelængde. 
...
Hyperspektral billeddannelse registrerer en intensitet som funktion af to rumlige koordinater og bølgelængde, dvs. en datakube $I(x,y,\lambda)$.
    ...
    Dette er i det ideelle scenarie, og ignorerer blandt andet, varmen genereret fra bredbåndskilden og prisen af NIR sensitive kameraer.
...
Det pulsatile PPG-signal følger hjertets frekvens, typisk \SIrange{0.5}{2}{Hz} svarende til 30 til 120 slag per minut. Systemet anvender tidsmultipleksing med tre tilstande (\SI{660}{nm}, \SI{940}{nm} og mørke), kørt med en samplingfrekvens, sat på baggrund af det brugte udstyr, f.eks. \SI{20}{kHz}. 
Hver kanal samples dermed med $\frac{20000}{3} \approx 6666 \ \text{Hz}$, hvilket er langt over Nyquist-grænsen for PPG-signalet og giver god tidsopløsning af pulsformen.
...
Melanin absorberer især i det synlige område og kan derfor påvirke den røde kanal mere end den infrarøde. Neglelak kan absorbere eller sprede lys afhængigt af farve og materiale og kan dermed ændre både DC-niveau og ratio-of-ratios \cite{Pologe2025}.

  
  \subsection{Dyshæmoglobiner}

Det klassiske to-bølgelængdeprincip antager, at hæmoglobin udelukkende eksisterer i enten iltet ($\text{HbO}_2$) eller uiltet ($\text{Hb}$) form. I praksis forekommer andre hæmoglobinformer, kaldet dyshæmoglobiner, som interfererer med målingen.

Kulilteforgiftning medfører dannelse af carboxyhæmoglobin (HbCO), som har en absorptionsprofil der ligner $\text{HbO}_2$ nær \SI{660}{nm}. Et pulsoximeter vil fejlaflæse HbCO som oxyhæmoglobin og dermed overestimere SpO$_2$, hvilket er klinisk kritisk.

Methæmoglobin (MetHb) har ved høje koncentrationer en tendens til at drive $R$ mod 1, svarende til SpO$_2$ $\approx$ 85\,\%, uanset den faktiske iltmætning.

Disse fejlkilder kan ikke kvantificeres pålideligt med et klassisk to-bølgelængdesystem og kræver co-oximetri med flere bølgelængder for at kvantificere de individuelle hæmoglobinformer.
  
  \subsection{Temperaturafhængighed af LED og fotodiode}

Temperaturen påvirker både lyskildens og detektorens egenskaber. For LED'er forskydes emissionsmaksimum mod længere bølgelængder med stigende temperatur, typisk \SIrange{0.1}{0.3}{nm/K} for AlGaInP-baserede røde LED'er. En temperaturstigning på \SI{30}{K} kan dermed forskyde emissionsmaksimumet med op til \SI{9}{nm}, hvilket ændrer de effektive ekstinktionskoefficienter og introducerer en systematisk fejl i $R$.

Fotodiodens mørkestrøm fordobles approksimativt for hver \SI{10}{K} temperaturstigning. Ved stuetemperatur er bidraget typisk negligibelt sammenlignet med fotostrømmen, men ved lav belysning eller høj omgivelsestemperatur kan mørkestrømmen påvirke det målte DC-niveau.
