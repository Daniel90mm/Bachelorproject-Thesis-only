\chapter{Appendix}


\begin{figure}[htbp]
     \centering
     % Første Billede (Øverst)
     \begin{subfigure}[b]{\textwidth}
         \centering
         \includegraphics[width=0.8\textwidth]{Pictures/PCB/TIA_LOWPASS.png}
         \caption{Skematisk diagram af Trans Impedance Amplifier(TIA) og lavpasfilter.}
         \label{fig:TIA_LP}
     \end{subfigure}
     
     \vspace{1cm} % Tilføjer vertikal afstand mellem de to billeder

     % Andet Billede (Nederst)
     \begin{subfigure}[b]{\textwidth}
         \centering
         \includegraphics[width=0.8\textwidth]{Pictures/PCB/HIGHPASS_GAIN.png}
         \caption{Skematisk diagram af højpasfilter og den ikke-inverterende forstærker.}
         \label{fig:HP_GAIN}
     \end{subfigure}
     
     \caption{Samlet oversigt over de analoge kredsløbstrin: TIA, filtrering og forstærkning.}
     \label{fig:circuit_stages}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/PCB/PCB.png}
    \caption{Den færdige PCB med traces forbundet.}
    \label{fig:PCB}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/PCB/PCB3D.png}
    \caption{3D-preview af PCB uden komponenter loddet.}
    \label{fig:PCB_3D}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Datasheets/AD3_Digital.png}
    \caption{Analog Discovery 3 digital output datasheet information.}
    \label{fig:ad3_datasheet}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Datasheets/2N7000_off_on_time.png}
    \caption{2N7000 MOSFET on/off tid fra datasheet fra Mouser.}
    \label{fig:2n7000_datasheet}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Datasheets/2N7000_onresistance.png}
    \caption{2N7000 MOSFET gatespænding og tilsvarende ON-modstand og strømstyrke igennem drain.}
    \label{fig:2N7000_onresistance}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Datasheets/2N7000_transfer_karakteristik.png}
    \caption{2N7000 MOSFET transferkarakteristik.}
    \label{fig:2N7000_transfer_karakteristik}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Datasheets/LVF.png}
    \caption{Relativ transmission i \% for det lineære variable filter (LF103924).}
    \label{fig:LVF}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Graphs/led_spectra_normalized_comparison.pdf}
    \caption{LED spektrum for SMD- og THT-LED'er. Bemærk en "hot" pixel ved ~\SI{980}{nm}, denne skal ignoreres.}
    \label{fig:LED_spektrum}
\end{figure}

\begin{table}[!ht]
    \centering
    \small
    \begin{tabular}{@{}lrrrrr@{}}
        \toprule
        LED & Nominel [nm] & Peak [nm] & Afvigelse [nm] & FWHM [nm] & Maks. int. [counts] \\
        \midrule
        \SI{660}{nm} & 660 & 664.99 & 4.99 & 18.30 & 44.83 \\
        \SI{810}{nm} & 810 & 806.73 & -3.27 & 20.98 & 43.59 \\
        \SI{940}{nm} & 940 & 951.79 & 11.79 & 27.25 & 38.79 \\
        \SI{970}{nm} & 970 & 965.58 & -4.42 & 40.00 & 34.37 \\
        \bottomrule
    \end{tabular}
    \caption{Tabel over LED peaks og deres afvigelser.}
    \label{tab:LED_bølgelængder}
\end{table}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Graphs/Finger_spektrum.pdf}
    \caption{Spektrum fra gennemlysning af finger med halogenpære.}
    \label{fig:finger_gennemlysning}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Graphs/Braun_pulsoximeter.pdf}
    \caption{Spektrum af det kommercielle Braun pulsoximeter}
    \label{fig:Braun_pulsox}
\end{figure}

\begin{table}[!ht]
    \centering
    \scriptsize
    \begin{tabular}{@{}lrrrrrrrr@{}}
        \toprule
        $\lambda$ & Stage [mm] & Output [counts/s] & Glo-bar [W/m$^2$] & L1 & L2 & L3 & LVF & QE \\
        \midrule
        \SI{800}{nm} & 5.880 & 1052.6 & 1.6630 & 0.9893 & 0.9885 & 0.9885 & 0.830 & 0.48 \\
        \SI{810}{nm} & 5.284 & 1157.9 & 1.8314 & 0.9893 & 0.9873 & 0.9875 & 0.840 & 0.47 \\
        \SI{820}{nm} & 4.688 & 1263.2 & 2.0042 & 0.9893 & 0.9865 & 0.9866 & 0.850 & 0.45 \\
        \SI{930}{nm} & -1.871 & 2000.0 & 4.8737 & 0.9892 & 0.9730 & 0.9728 & 0.930 & 0.24 \\
        \SI{940}{nm} & -2.467 & 1789.5 & 5.2153 & 0.9892 & 0.9715 & 0.9715 & 0.925 & 0.22 \\
        \SI{950}{nm} & -3.063 & 1263.2 & 5.5886 & 0.9892 & 0.9700 & 0.9700 & 0.920 & 0.18 \\
        \bottomrule
    \end{tabular}
    \caption{Spektrale korrektionsfaktorer anvendt til hyperspektral korrektion. L1--L3 betegner de tre linsetransmissioner, LVF betegner filtertransmissionen, og QE betegner kameraets kvanteeffektivitet.}
    \label{tab:hyperspectral_correction}
\end{table}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Graphs/hpf_filter_bode.pdf}
    \caption{Frekvenskarakteristik af HPF}
    \label{fig:hpf_filter_bode_appendix}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Pictures/Other/led_driver_schematic.png}
    \caption{LED driver skema}
    \label{fig:led_driver_schematic}
\end{figure}






